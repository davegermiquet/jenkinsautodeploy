...
RUN apt-get update && apt-get install -y docker-ce-cli terraform wget rsync git python3-pip python3-dev python3 curl apt-utils  software-properties-common golang net-tools dnsmasq postgresql-client openssh-server 
...
RUN ssh-keygen -N "" -b 4028 -f /var/jenkins_home/.ssh/id_rsa